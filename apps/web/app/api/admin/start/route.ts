...
export async function POST(req: NextRequest) {
  ...
  // Live ได้ทีละ 1 ต่อ tenant — ถ้าจะ start เป็น Live ต้องไม่มี Live อื่นค้างใน tenant เดียวกัน
  if (mode === 'live') {
    const current = activeRun(tenantId);
  ...
  // ถ้ามี run ของ folder นี้อยู่แล้วใน tenant เดียวกัน → reuse, ถ้าไม่มี → สร้างใหม่
  const existing = db()
  ...
  if (existing) {
    db()
      ...
      .run(mode, 'running', folderName, existing.id);
    runId = existing.id;
  } else {
    const r = db()
      .prepare(`INSERT INTO runs (folder_id, folder_name, started_at, status, mode, tenant_id) VALUES (?, ?, ?, ?, ?, ?)`)
      .run(folderId, folderName, now, 'running', mode, tenantId);
    runId = Number(r.lastInsertRowid);
  }

  // Live: auto-enable QR ให้ทันที (public, no password)
  let autoQR: { code: string; created: boolean } | null = null;
  if (mode === 'live') {
    autoQR = ensureEventCodeForFolder(folderId, tenantId);
