...
import { allEmbeddings, db, activeRun } from '@/lib/db';
...
export async function POST(req: NextRequest) {
  const run = activeRun();
  if (!run) return NextResponse.json({ error: 'no active run' }, { status: 400 });
  ...
  const rows = allEmbeddings(run.id);

  type Hit = { photoId: string; distance: number };
  ...
  const hits: Hit[] = Array.from(byPhoto.entries())
